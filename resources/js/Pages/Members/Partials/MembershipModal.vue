<template>
  <modal :show="show" max-width="sm" @close="$emit('close')">
    ...
    <div class="px-6 py-4 text-xl font-medium">
      {{ member.membership ? 'Edit Membership' : 'Create Membership'}}
    </div>
  ...
  watch: {
    show(val) {
      ...
      this.form = this.$inertia.form({
        type: this.member.membership?.type,
        plan_type_id: this.member.membership?.plan_type_id,
      })
    }
  },
  ...
  methods: {
    submit() {
      if (this.member.membership) {
        this.form.put(this.$route('memberships.update', this.member.membership.id), {
